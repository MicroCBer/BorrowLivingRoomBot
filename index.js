...
let initUserData = { cookie: null, oauthKey: null, allowUses: [] };
function to_form(details) {
    var formBody = [];
...
async function main() {
    ...
    bot.onMessage(async function (data) {
        if (!is(data.sender, "group"))
            return;
        if (!data.messageChain[0].id)
            return;
        if (!db.data)
            return;
        let message = {
        ...
        for (let msg of data.messageChain) {
            if (msg.text) {
                if (message.command) {
                    ...
                    let firstword = msg.text.trim().split(" ")[0], firstchar = firstword.charAt(0);
        ...
        switch (message.command) {
            case "bl:login": {
                let login_data = await (await fetch("https://passport.bilibili.com/qrcode/getLoginUrl")).json();
                if (!db.data.login_data[message.sender])
                    db.data.login_data[message.sender] = initUserData;
            ...
            case "bl:startLive": {
                ...
                let resp = await (await fetch("https://api.live.bilibili.com/room/v1/Room/startLive", {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/x-www-form-urlencoded;charset=UTF-8",
                        "Cookie": parsed.all
            ...
            case "bl:stopLive": {
                ...
                if (!parsed)
                    return;
                let roomid = await getRoomID(parsed.all);
                let resp = await (await fetch("https://api.live.bilibili.com/room/v1/Room/stopLive", {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/x-www-form-urlencoded;charset=UTF-8",
                        "Cookie": parsed.all
                ...
                if (resp.code)
                    await bot.sendGroupMessage([Plain("请求失败！\n返回值：" + JSON.stringify(resp))], message.group);
                else {
...
async function getRoomID(cookie) {
    let parsed = parseCookie(cookie);
    if (!parsed)
        throw Error("NO COOKIE");
    let userdata = await (await fetch(`https://api.bilibili.com/x/web-interface/nav`, {
        headers: { Cookie: parsed.all }
    })).json();
    let roomdata = await (await fetch(`https://api.live.bilibili.com/live_user/v1/Master/info?uid=${userdata.data.mid}`)).json();
